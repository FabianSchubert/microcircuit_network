...
w_update_model_plast = {
    ...
    "sim_code": f"""
        // SIM CODE PP_basal
        $(dg) += $(muPP_basal) * ($(t) - max(max(0.0,$(prev_sT_pre)),$(sT_post))) * $(r_prev_prev_pre) * ($(r_prev_post) - $(r_eff_prev_post));
    """,
    "learn_post_code": f"""
        // LEARN POST CODE IP
        $(dg) += $(muPP_basal) * ($(t) - max(max(0.0,$(prev_sT_post)),$(sT_pre))) * $(r_prev_pre) * ($(r_prev_prev_post) - $(r_eff_prev_prev_post));
    """,
...
wu_param_space_plast = {"muPP_basal": 1e-2}
