...
params_list.extend(product([models.items()[0]], method_params.items(),
                           NET_SIZE_ZIP, SPIKE_THRESHOLDS, range(N_RUNS)))

params_list.extend(product([models.items()[1], method_params.items(),
...
with open(os.path.join(BASE_FOLD, "runtime_est.log"), "a") as file_log:
    ...
    for k_sweep, ((model_name, model), (method_name, method_param), net_size_zip, spike_th, sim_id) in enumerate(sweep_params_instance):
